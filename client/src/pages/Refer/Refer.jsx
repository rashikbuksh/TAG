...
const Refer = () => {
  ...
  useEffect(() => {
    if (user) {
      const id = user.id;
      api
        .get(`/profile/check_refer_status/${id}`)
        .then((response) => {
  ...
  const handelHaveNotReferCode = () => {
    ...
    api
      .post(`/profile/edit_refer_status/${id}`, {
    ...
    <>
      <Modal isOpen={isOpen} setIsOpen={setIsOpen}>
        ...
        <div className="flex items-center justify-center space-x-11 p-2">
          ...
          <button onClick={handelHaveReferCode} className="btn btn-xs">
            Yes
          </button>
          <button onClick={handelHaveNotReferCode} disabled={isSubmitting}  className="btn btn-xs">
            No
          </button>
        </div>
        {haveReferCode && (
          <div className="flex flex-col items-center justify-center">
            ...
            {formData.errors && formData.errors.referCode && (
              <p className="text-red-500">{formData.errors.referCode}</p>
            )}
            <button
