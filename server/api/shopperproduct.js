...
const read = [
  ...
  },
  {
    ...
    query: `SELECT sp.id, sp.name, sp.price, discount, product_count, product_id, isVerified, category_id, p.image, p.optionalImage1,p.optionalImage2,p.keywords, p.title as title, sp.shopper_id, sale_count, sp.view, p.full_description , p.short_description, cp.shipping_address, cp.active_status,cp.access FROM shopper_product sp, product p, customer_profile cp WHERE sp.product_id = p.id AND cp.id = sp.shopper_id and sp.id = ?`,
    param: ["id"],
  },
  {
  ...
  },
  {
    ...
    query: `SELECT 
            sp.id, sp.name,sp.shopper_id, sp.price, discount, product_count, product_id, category_id, p.image,p.title as title,p.isVerified, sp.view, cp.shipping_address,cp.access 
            FROM shopper_product sp, product p, customer_profile cp 
            WHERE sp.product_id = p.id and cp.id = sp.shopper_id and shopper_id = ?`,
    param: ["id"],
  },
  {
  ...
  },
  {
    ...
    query: `SELECT 
              sp.id, sp.name, sp.price, sp.discount, sp.product_count, isVerified, product_id, category_id, p.image,p.title as title, sp.shopper_id, sp.sale_count, sp.view, cp.active_status,cp.access, cp.shipping_address
            FROM shopper_product sp
              JOIN product p ON sp.product_id = p.id
  ...
  },
  {
    ...
    query: `SELECT 
              sp.id, sp.name, sp.price, sp.discount, sp.product_count, sp.product_id, p.isVerified, p.category_id, p.image, p.title as title, sp.shopper_id, sp.sale_count, sp.view, cp.shipping_address,cp.access, cp.active_status 
            FROM shopper_product sp 
              JOIN product p ON sp.product_id = p.id JOIN customer_profile cp ON cp.id = sp.shopper_id WHERE sp.product_count > 0 AND cp.access = 'admin';
`,
  },
];
